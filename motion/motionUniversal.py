...
__name__ = "motionUniversal"
__version__ = "v111"

import argparse
from Camera import Camera, PixelFormat
...
def draw_box(db_frame, db_label, db_contour):
    ...
    cv2.putText(db_frame, db_label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    return db_frame
...
if __name__ == "motionUniversal":
    ...
    framerate = int(get_parameter(parser, 'framerate', 20))
    ...
    # Start Camera
    cam = Camera()
    ret = cam.start_camera(width, height, framerate)
        ...
        while not killer.kill_now:
            ...
            if len(contours) > 0:
                ...
                if not box == 'OFF':
                    ...
                    draw_box(frame, box_text, contour)
            ...
            if frames_required > 0:
                if statistics and frames_required < 10:
                    stats = 'Sensitivity: {}\nTotal Frames: {}\nPre Movement Frames: {}\nPost Movement Frames: {}'.format(
                        sensitivity, frames_written, pre_frames, post_frames)
                    buffered_frame = put_text(buffered_frame, stats)
                    jpg_frame = buffered_frame
                if display:
